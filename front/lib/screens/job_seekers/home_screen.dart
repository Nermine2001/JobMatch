...
class HomeScreen extends StatefulWidget {
  ...
  const HomeScreen({super.key, this.filters });
  ...
  Stream<QuerySnapshot> getFilteredJobs({String? sortBy}) {
    ...
    if (filters != null) {
      ...
      if (filters!['minSalary'] != null && filters!['minSalary'].isNotEmpty) {
        ...
        if (minSalary != null) query = query.where('salary', isGreaterThanOrEqualTo: minSalary);
      }

      if (filters!['maxSalary'] != null && filters!['maxSalary'].isNotEmpty) {
        final maxSalary = int.tryParse(filters!['maxSalary']);
        if (maxSalary != null) query = query.where('salary', isLessThanOrEqualTo: maxSalary);
      }

      if (filters!['minExperience'] != null) {
        query = query.where('experience', isGreaterThanOrEqualTo: filters!['minExperience']);
      }

      if (filters!['maxExperience'] != null) {
        query = query.where('experience', isLessThanOrEqualTo: filters!['maxExperience']);
      }

      if (filters!['minAge'] != null && filters!['minAge'].isNotEmpty) {
        final minAge = int.tryParse(filters!['minAge']);
        if (minAge != null) query = query.where('age', isGreaterThanOrEqualTo: minAge);
      }

      if (filters!['maxAge'] != null && filters!['maxAge'].isNotEmpty) {
        final maxAge = int.tryParse(filters!['maxAge']);
        if (maxAge != null) query = query.where('age', isLessThanOrEqualTo: maxAge);
      }
    ...
    Widget build(BuildContext context) {
      ...
      return Scaffold(
        ...
        appBar: AppBar(
          ...
          bottom: _titles[_selectedIndex] == 'Looking for a work' ? PreferredSize(
                      ...
                      onTap: () async {
                        ...
                        if (selectedFilters != null) {
                          Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(builder: (_) => HomeScreen(filters: selectedFilters)),
                          );
                        }
                      },
...
class _HomeTabScreenState extends State<HomeTabScreen> {
  ...
  Widget _buildFilterChip({required IconData icon, required String label, required String sortField}) {
      ...
      child: Column(
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              _buildFilterChip(icon: Icons.location_on_outlined, label: 'Location', sortField: 'location'),
              SizedBox(width: 12),
              _buildFilterChip(icon: Icons.work_outline, label: 'Job Type', sortField: 'type'),
              SizedBox(width: 12,),
              _buildFilterChip(icon: Icons.schedule_outlined, label: 'Period', sortField: 'period'),
              SizedBox(width: 12),
              _buildFilterChip(icon: Icons.attach_money_outlined, label: 'Cost', sortField: 'salary'),
            ],
          ),
          SizedBox(height: 15,),
          Expanded(
